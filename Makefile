...
lint:  ## run static analysis with flake8
	python -m black --check zkpy setup.py
	python -m flake8 zkpy setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black zkpy/ setup.py
...
test: ## clean and run unit tests
	python -m pytest -v zkpy/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v zkpy/tests --cov=zkpy --cov-branch --cov-fail-under=75 --cov-report term-missing
